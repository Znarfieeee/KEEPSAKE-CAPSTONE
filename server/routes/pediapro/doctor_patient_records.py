...
def get_patient_record_by_id(patient_id):
    ...
    With Redis caching for faster subsequent retrievals.
    """
    try:
        include_related = request.args.get('include_related', 'false').lower() == 'true'
        
        cache_key = f"{PATIENT_CACHE_PREFIX}{patient_id}"
        
        cached_data = redis_client.get(cache_key)
        if cached_data:
            patient_data = json.loads(cached_data)
            return jsonify({
                "status": "success",
                "data": patient_data,
                "cached": True,
        ...
        # If not in cache or busting cache, get from database
        ...
        # Cache the complete patient data
