...
import {
  TbHeartbeat, TbVaccine, TbPill, TbCalendar,
  TbAlertTriangle, TbRuler, TbArrowLeft
} from 'react-icons/tb'
        ...
        <CardContent className="pt-6">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            <div className="flex items-start gap-4">
              <div className={`p-4 rounded-full ${childData.sex === 'male' ? 'bg-blue-100' : 'bg-pink-100'}`}>
                {childData.sex === 'male' ? (
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">
                  {childData.firstname} {childData.middlename ? `${childData.middlename} ` : ''}{childData.lastname}
                </h1>
                <p className="text-gray-600 mt-1">{childData.age || 'Age not calculated'}</p>
                <div className="flex flex-wrap gap-2 mt-3">
                  <Badge variant="outline" className="capitalize">
                    {childData.relationship}
                  </Badge>
                  <Badge variant={childData.sex === 'male' ? 'default' : 'secondary'}>
                    {childData.sex === 'male' ? 'Male' : 'Female'}
                  </Badge>
                  {childData.bloodtype && (
                    <Badge variant="outline">Blood Type: {childData.bloodtype}</Badge>
                  )}
                </div>
              </div>
            </div>
            <div className="text-sm text-gray-600 space-y-1">
              <p><span className="font-medium">Date of Birth:</span> {new Date(childData.date_of_birth).toLocaleDateString()}</p>
              {childData.birth_weight && (
                <p><span className="font-medium">Birth Weight:</span> {childData.birth_weight} kg</p>
              )}
              {childData.birth_height && (
                <p><span className="font-medium">Birth Height:</span> {childData.birth_height} cm</p>
              )}
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-yellow-200 bg-yellow-50">
        <CardContent className="pt-4">
          <p className="text-sm text-yellow-800 flex items-center gap-2">
            ...
            <span>This is a read-only view. Contact your healthcare provider for any updates or changes.</span>
          </p>
        </CardContent>
      </Card>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  {delivery.type_of_delivery && (
                    <div>
                      ...
                      <p className="font-medium capitalize">{delivery.type_of_delivery}</p>
                    </div>
                  )}
                  ...
                  {delivery.gestation_weeks && (
                    <div>
                      ...
                      <p className="font-medium">{delivery.gestation_weeks} weeks</p>
                    </div>
                  )}
                  {delivery.distinguishable_marks && (
                    <div className="col-span-2">
                      <span className="text-gray-600">Distinguishable Marks:</span>
                      <p className="font-medium">{delivery.distinguishable_marks}</p>
                    </div>
                  )}
                </div>
              </CardContent>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  {anthropometric[0].weight && (
                    <div>
                      ...
                      <p className="font-medium text-lg">{anthropometric[0].weight} kg</p>
                    </div>
                  )}
                  {anthropometric[0].height && (
                    <div>
                      <span className="text-gray-600">Height:</span>
                      <p className="font-medium text-lg">{anthropometric[0].height} cm</p>
                    </div>
                  )}
                  {anthropometric[0].head_circumference && (
                    <div>
                      <span className="text-gray-600">Head Circumference:</span>
                      <p className="font-medium text-lg">{anthropometric[0].head_circumference} cm</p>
                    </div>
                  )}
                  {anthropometric[0].measurement_date && (
                    <div className="col-span-2 md:col-span-4 text-xs text-gray-500">
                      Measured on: {new Date(anthropometric[0].measurement_date).toLocaleDateString()}
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  {screening.ens_date && (
                    <div>
                      <span className="text-gray-600">Expanded Newborn Screening:</span>
                      <p className="font-medium">
                        {new Date(screening.ens_date).toLocaleDateString()}
                        {screening.ens_remarks !== null && (
                          <Badge variant={screening.ens_remarks ? 'default' : 'destructive'} className="ml-2">
                  ...
                  {screening.nhs_date && (
                    <div>
                      <span className="text-gray-600">Newborn Hearing Screening:</span>
                      <p className="font-medium">
                        {new Date(screening.nhs_date).toLocaleDateString()}
                      </p>
                      {(screening.nhs_right_ear !== null || screening.nhs_left_ear !== null) && (
                        <p className="text-xs mt-1">
                          Right: {screening.nhs_right_ear ? 'Pass' : 'Refer'} |
                          Left: {screening.nhs_left_ear ? 'Pass' : 'Refer'}
                        </p>
                      )}
                    </div>
                  )}
                  ...
                  <CardContent className="pt-6">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 capitalize">{allergy.reaction_type}</p>
                      </div>
                      <Badge variant={
                        allergy.severity === 'life_threatening' ? 'destructive' :
                    ...
                    {allergy.notes && (
                      <p className="text-sm text-gray-700 mb-2">{allergy.notes}</p>
                    )}
              ...
              {vaccinations.map((vax) => (
                <Card key={vax.vax_id}>
                  <CardContent className="pt-6">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <h3 className="font-semibold text-lg">{vax.vaccine_name}</h3>
                        {vax.dose_number && (
                          <p className="text-sm text-gray-600">Dose #{vax.dose_number}</p>
                        )}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-sm">
                      {vax.manufacturer && (
                        <div>
                          ...
                          <p className="font-medium">{vax.manufacturer}</p>
                        </div>
                      )}
                      {vax.administration_site && (
                        <div>
                          <span className="text-gray-600">Site:</span>
                          <p className="font-medium capitalize">{vax.administration_site}</p>
                        </div>
                      )}
                      {vax.next_dose_due && (
                        <div className="col-span-2 mt-2 p-2 bg-blue-50 rounded">
                          <span className="text-blue-800 text-sm">
                            Next dose due: {new Date(vax.next_dose_due).toLocaleDateString()}
                          </span>
                        </div>
                      )}
                    </div>
                    {vax.notes && (
                      <p className="text-sm text-gray-700 mt-3">{vax.notes}</p>
                    )}
                  </CardContent>
                </Card>
              ))}
                  ...
                  <CardContent className="pt-6">
                      ...
                      <div>
                        ...
                        {rx.users && (
                          <p className="text-sm text-gray-600">
                            ...
                            {rx.users.specialty && ` - ${rx.users.specialty}`}
                          </p>
                        )}
                      </div>
                      <Badge variant="outline">
                    ...
                    {rx.findings && (
                      <div className="mb-3">
                        ...
                        <p className="text-sm text-gray-600 mt-1">{rx.findings}</p>
                      </div>
                    )}
                    {rx.doctor_instructions && (
                      <div className="mb-3">
                        <span className="text-sm font-medium text-gray-700">Instructions:</span>
                        <p className="text-sm text-gray-600 mt-1">{rx.doctor_instructions}</p>
                      </div>
                    )}
                    {rx.return_date && (
                      <div className="mt-3 p-2 bg-green-50 rounded">
                        <span className="text-green-800 text-sm">
                          Follow-up: {new Date(rx.return_date).toLocaleDateString()}
                        </span>
                      </div>
                    )}
                  </CardContent>
                  ...
                  <CardContent className="pt-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                      {measurement.weight && (
                        <div>
                          <span className="text-gray-600">Weight:</span>
                          <p className="font-medium text-lg">{measurement.weight} kg</p>
                        </div>
                      )}
                      {measurement.height && (
                        <div>
                          <span className="text-gray-600">Height:</span>
                          <p className="font-medium text-lg">{measurement.height} cm</p>
                        </div>
                      )}
                      {measurement.head_circumference && (
                        <div>
                          <span className="text-gray-600">Head:</span>
                          <p className="font-medium text-lg">{measurement.head_circumference} cm</p>
                        </div>
                      )}
                      {measurement.chest_circumference && (
                        <div>
                          <span className="text-gray-600">Chest:</span>
                          <p className="font-medium text-lg">{measurement.chest_circumference} cm</p>
                        </div>
                      )}
                      {measurement.abdominal_circumference && (
                        <div>
                          <span className="text-gray-600">Abdomen:</span>
                          <p className="font-medium text-lg">{measurement.abdominal_circumference} cm</p>
                        </div>
                      )}
                    </div>
                  </CardContent>
                  ...
                  <CardContent className="pt-6">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        {appt.users && (
                          <p className="text-sm text-gray-600">
                            Dr. {appt.users.firstname} {appt.users.lastname}
                            {appt.users.specialty && ` - ${appt.users.specialty}`}
                          </p>
                        )}
                      </div>
                      <Badge variant={
