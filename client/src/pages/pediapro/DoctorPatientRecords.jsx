...
import { usePatientsRealtime, supabase } from '@/hook/useSupabaseRealtime'
import {
  getPatientById,
  addPatientRecord,
  updatePatientRecord,
...
function DoctorPatientRecords() {
      ...
      switch (type) {
        case 'INSERT':
          setPatients((prev) => {
            ...
            showToast('success', `New patient "${patient.firstname} ${patient.lastname}" added`)
            return [...prev, patient]
          })
          break
        ...
        case 'DELETE':
          ...
          showToast('warning', `Patient "${patient.firstname} ${patient.lastname}" removed`)
  ...
  const handleAddPatient = async () => {
    const response = await addPatientRecord()
  ...
  const handleView = (record) => {
    // Navigate directly with the record data since we already have real-time updates
