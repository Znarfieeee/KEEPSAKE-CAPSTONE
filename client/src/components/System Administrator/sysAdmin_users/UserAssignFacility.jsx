...
const UserAssignFacility = ({ open, onClose, userId, user }) => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      showToast('error', 'Failed to assign user to facility. Please try again.')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[400px] border-0 shadow-sm bg-white [&_button[data-slot=dialog-close]]:text-transparent [&_button[data-slot=dialog-close]]:p-1">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Assign Facility</DialogTitle>
          <span className="flex gap-1">
            <p className="text-sm text-black">Assigning facility for</p>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-4">
            ...
            <DialogFooter className="w-full mt-6 justify-between">
              ...
              <Button type="submit" disabled={loading} className="bg-primary text-white px-4">
