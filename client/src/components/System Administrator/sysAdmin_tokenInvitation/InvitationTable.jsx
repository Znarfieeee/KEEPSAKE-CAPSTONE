...
const InvitationTable = ({
  ...
  const getFacilityName = (facilityId) => {
    return facilities?.find((f) => f.facility_id === facilityId)?.facility_name || 'N/A'
  }
    ...
    <div className="w-full overflow-x-auto">
        ...
        <thead className="border-b border-gray-300 text-xs-uppercase text-muted-foreground">
          <tr className="text-left">
            ...
            <th className="py-3 px-2">
              Expires <span className="text-xs font-medium text-gray-400">(YYYY-MM-DD)</span>
            </th>
                ...
                <tr key={user.id} className="border-b border-gray-200 last:border-none">
                  ...
                  <td className="p-2 whitespace-nowrap">
                    <div className="flex gap-1">
                      ...
                      <TooltipHelper
                        content={user.status === 'inactive' ? 'Activate User' : 'Deactivate User'}
                      >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-4">
        ...
        <div className="flex items-center gap-2 text-sm">
          <span>
            {startIdx + 1}-{Math.min(startIdx + itemsPerPage, invitations.length)} of{' '}
            {invitations.length}
          </span>
          <Button size="icon" variant="ghost" onClick={handlePrev} disabled={page === 1}>
            <ChevronLeft className="size-4" />
          </Button>
          <Button size="icon" variant="ghost" onClick={handleNext} disabled={page === totalPages}>
